...
class Setup(commands.Cog):
    ...
    async def autoRole(self, ctx):
        ...
        if defRole:
            auto = 'set to: **{}**.'.format(
                DisplayName.roleForID(defRole, server))
        else:
            auto = '*disabled*.'
    ...
    async def autoRoleName(self, ctx):
        ...
        while not gotIt:
            ...
            if not talk:
                ...
                if not role:
                    msg = "It doesn't look like **{}** is a role on your server - try again.".format(
                        talk.content)
                    await author.send(msg)
                    continue
                else:
                ...
                if talk.content.lower() == "skip":
                    ...
                    try:
                        ...
                        self.settings.setServerStat(
                            server, "VerificationTime", talkInt)
                    except ValueError:
    ...
    async def setupXP(self, ctx):
        ...
        while not gotIt:
            ...
            if not talk:
                ...
                if talk.content.lower() == "skip":
                    ...
                    self.settings.setServerStat(
                        server, "DefaultXPReserve", defXPR)
                else:
                    try:
                        talkInt = int(talk.content)
                        await author.send('Default xp reserve is now *{}!*'.format(talkInt))
                        self.settings.setServerStat(
                            server, "DefaultXPReserve", talkInt)
                    except ValueError:
                        # await self.autoRoleName(ctx)
        ...
        while not gotIt:
            ...
            if not talk:
                ...
                if talk.content.lower() == "skip":
                    ...
                    self.settings.setServerStat(
                        server, "HourlyXPReal", hourXPReal)
                else:
                    try:
                        talkInt = int(talk.content)
                        await author.send('Hourly xp is now *{}!*'.format(talkInt))
                        self.settings.setServerStat(
                            server, "HourlyXPReal", talkInt)
                    except ValueError:
                        # await self.autoRoleName(ctx)
        ...
        while not gotIt:
            ...
            if not talk:
                ...
                if talk.content.lower() == "skip":
                    ...
                    self.settings.setServerStat(
                        server, "XPPerMessage", messageXP)
                else:
                    try:
                        talkInt = int(talk.content)
                        await author.send('Xp per message is now *{}!*'.format(talkInt))
                        self.settings.setServerStat(
                            server, "XPPerMessage", talkInt)
                    except ValueError:
                        # await self.autoRoleName(ctx)
        ...
        while not gotIt:
            ...
            if not talk:
                ...
                if talk.content.lower().startswith('y'):
                    self.settings.setServerStat(
                        server, "SuppressPromotions", True)
                    await author.send('I will avoid sending a promotion message.')
                elif talk.content.lower().startswith('n'):
                    self.settings.setServerStat(
                        server, "SuppressPromotions", False)
        ...
        while not gotIt:
            ...
            if not talk:
                ...
                if talk.content.lower().startswith('y'):
                    self.settings.setServerStat(
                        server, "SuppressDemotions", True)
                    await author.send('I will avoid sending a demotion message.')
                elif talk.content.lower().startswith('n'):
                    self.settings.setServerStat(
                        server, "SuppressDemotions", False)
        ...
        xpDem = self.settings.getServerStat(server, "XPDemote")
        if xpProm or xpDem:
            msg = 'To set up your xp promotion/demotion roles - use the `{}addxprole [role] [required xp]` and `{}removexprole [role]` in the main chat.'.format(
            ...
            if not talk:
                ...
                if talk.content.lower() == "skip":
                    ...
                    try:
                        ...
                        self.settings.setServerStat(
                            server, "PictureThreshold", talkInt)
                    except ValueError:
                        # await self.autoRoleName(ctx)
            ...
            if not talk:
                ...
                if talk.content.lower() == "skip":
                    ...
                    try:
                        ...
                        self.settings.setServerStat(
                            server, "DefaultVolume", (talkInt/100))
                    except ValueError:
                        # await self.autoRoleName(ctx)
