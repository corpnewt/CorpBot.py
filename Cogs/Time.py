...
class Time(commands.Cog):
    ...
    async def settz(self, ctx, *, tz: str = None):
        ...
        if not tz_list[0]['Ratio'] == 1:
            ...
            self.settings.setGlobalUserStat(
                ctx.author, "TimeZone", tz_list[index]['Item'])
            await message.edit(content="TimeZone set to `{}`!".format(tz_list[index]['Item']))
            return
        # We got a time zone
        self.settings.setGlobalUserStat(
            ctx.author, "TimeZone", tz_list[0]['Item'])
    ...
    async def tz(self, ctx, *, member=None):
        ...
        msg = '*{}\'s* TimeZone is *{}*'.format(
            DisplayName.name(member), timezone)
        await ctx.channel.send(msg)
    ...
    async def setoffset(self, ctx, *, offset: str = None):
        ...
        if offset == None:
            self.settings.setGlobalUserStat(
                ctx.message.author, "UTCOffset", None)
            msg = '*{}*, your UTC offset has been removed!'.format(
        ...
        msg = '*{}*, your UTC offset has been set to `{}`!'.format(
            DisplayName.name(ctx.message.author), off)
        await ctx.channel.send(msg)
