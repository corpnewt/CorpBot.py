...
class Debugging(commands.Cog):
    ...
    def __init__(self, bot, settings, debug=False):
        ...
        self.logvars = ['invite.create', 'invite.delete', 'invite.send', 'user.ban', 'user.unban', 'user.mute', 'user.unmute', 'user.join', 'user.leave', 'user.status',
                        ...
                        'message.edit', "xp"]
        self.quiet = ['user.ban', 'user.unban', 'user.mute',
                      'user.unmute', 'user.join', 'user.leave']
    ...
    async def oncommand(self, ctx):
        if self.debug:
            ...
            msg = '{}{}:\n"{}"\nRun at {}\nBy {}\nOn {}'.format(
                ctx.prefix, ctx.command, ctx.message.content, timeStamp, ctx.author.name, ctx.guild.name)
    ...
    def format_invite(self, invite, sent=False):
        ...
        url = invite.url if invite.url else "https://discord.gg/{}".format(
            invite.code)
        expires_after = None if invite.max_age == None else "Never" if invite.max_age == 0 else "In " + \
            ReadableTime.getReadableTimeBetween(0, invite.max_age)
        max_uses = None if invite.max_uses == None else "Unlimited" if invite.max_uses == 0 else "{:,}".format(
            invite.max_uses)
        uses = None if invite.uses == None else "{:,}".format(invite.uses)
        created_by = None if invite.inviter == None else "{}#{} ({})".format(
            invite.inviter.name, invite.inviter.discriminator, invite.inviter.id)
        created_at = None if invite.created_at == None else invite.created_at.strftime(
            "%b %d %Y - %I:%M %p") + " UTC"
        ...
        if sent == True:
            ...
            if invite.approximate_member_count != None:
                desc += "\nUsers:           {}\{}".format(
                    invite.approximate_presence_count, invite.approximate_member_count)
        if created_by != None:
    ...
    async def on_xp(self, to_user, from_user, amount):
        ...
        pfpurl = from_user.avatar_url if len(
            from_user.avatar_url) else from_user.default_avatar_url
        if not self.shouldLog('xp', guild):
            return
        if type(to_user) is discord.Role:
            msg = "🌟 {}#{} ({}) gave {} xp to the {} role.".format(
                from_user.name, from_user.discriminator, from_user.id, amount, to_user.name)
        else:
            msg = "🌟 {}#{} ({}) gave {} xp to {}#{} ({}).".format(
                from_user.name, from_user.discriminator, from_user.id, amount, to_user.name, to_user.discriminator, to_user.id)
    ...
    async def on_member_ban(self, guild, member):
        if not self.shouldLog('user.ban', guild):
            return
        # A member was banned
        pfpurl = member.avatar_url if len(
            member.avatar_url) else member.default_avatar_url
        msg = '🚫 {}#{} ({}) was banned from {}.'.format(
            member.name, member.discriminator, member.id, guild.name)
    ...
    async def on_mute(self, member, guild, cooldown, muted_by):
        if not self.shouldLog('user.mute', guild):
            return
        # A memeber was muted
        pfpurl = member.avatar_url if len(
            member.avatar_url) else member.default_avatar_url
        msg = "🔇 {}#{} ({}) was muted.".format(
            member.name, member.discriminator, member.id)
        message = "Muted by {}.\nMuted {}.".format(
            "Auto-Muted" if not muted_by else "{}#{} ({})".format(
                muted_by.name, muted_by.discriminator, muted_by.id),
            "for "+ReadableTime.getReadableTimeBetween(
                time.time(), cooldown) if cooldown else "until further notice"
        )
    ...
    async def on_unmute(self, member, guild):
        if not self.shouldLog('user.unmute', guild):
            return
        # A memeber was muted
        pfpurl = member.avatar_url if len(
            member.avatar_url) else member.default_avatar_url
        msg = "🔊 {}#{} ({}) was unmuted.".format(
            member.name, member.discriminator, member.id)
        await self._logEvent(guild, "", title=msg, color=discord.Color.green(), thumbnail=pfpurl)
    ...
    async def on_invite_create(self, invite):
        ...
        if invite.inviter != None and len(invite.inviter.avatar_url):
            pfpurl = invite.inviter.avatar_url
        # Store the invite in our working list
        self.invite_list[str(guild.id)] = self.invite_list.get(
            str(guild.id), [])+[invite]
        if not self.shouldLog('invite.create', invite.guild):
            return
    ...
    async def on_invite_delete(self, invite):
        ...
        if not guild:
            return  # Didn't find it
        pfpurl = guild.icon_url if len(
            guild.icon_url) else self.bot.user.default_avatar_url
        # Refresh the list omitting the deleted invite
        self.invite_list[str(guild.id)] = [x for x in self.invite_list.get(
            str(guild.id), []) if x.code != invite.code]
        if not self.shouldLog('invite.delete', guild):
            return
        msg = "📤 Invite deleted."
        log_msg = self.format_invite(invite)
    ...
    async def on_member_join(self, member):
        ...
        try:
            ...
            new_invites = []
        changed = [x for x in new_invites for y in invite_list if x.code ==
                   y.code and x.uses != y.uses]
        if len(changed) == 1:
        ...
        msg = '👐 {}#{} ({}) joined {}.'.format(
            member.name, member.discriminator, member.id, guild.name)
        log_msg = "Account Created: {}".format(
            "Unknown" if member.created_at == None else member.created_at.strftime("%b %d %Y - %I:%M %p") + " UTC")
        if invite:
    ...
    async def on_member_update(self, before, after):
        ...
        server = before.guild
        pfpurl = before.avatar_url if len(
            before.avatar_url) else before.default_avatar_url
        if not before.status == after.status and self.shouldLog('user.status', server):
            msg = 'Changed Status:\n\n{}\n   --->\n{}'.format(
                str(before.status).lower(), str(after.status).lower())
        ...
        if not before.activities == after.activities:
            ...
            bact = [x for x in list(before.activities)
                    if not x in list(after.activities)]
            aact = [x for x in list(after.activities)
                    if not x in list(before.activities)]
            # Now we format
            changes = {}
            for x in bact:
            ...
            for k in changes:
                ...
                b = self.activity_to_dict(changes[k].get(
                    "before", discord.Activity(name=None, url=None, type=None)))
                a = self.activity_to_dict(changes[k].get(
                    "after", discord.Activity(name=None, url=None, type=None)))
    ...
    async def on_message(self, message):
        ...
        if self.shouldLog('message.send', message.guild):
            # A message was sent
            title = '📧 {}#{} ({}), in #{}, sent:'.format(
                message.author.name, message.author.discriminator, message.author.id, message.channel.name)
            msg = message.content
            if len(message.attachments):
                msg += "\n\n--- Attachments ---\n\n"
                for a in message.attachments:
                    msg += a.url + "\n"
            pfpurl = message.author.avatar_url if len(
                message.author.avatar_url) else message.author.default_avatar_url
    ...
    async def on_message_edit(self, before, after):
        ...
        title = '✏️ {}#{} ({}), in #{}, edited:'.format(
            before.author.name, before.author.discriminator, before.author.id, before.channel.name)
        msg = before.content
        ...
        if len(after.attachments):
            ...
            for a in after.attachments:
                msg += a.url + "\n"
        pfpurl = before.author.avatar_url if len(
            before.author.avatar_url) else before.author.default_avatar_url
    ...
    async def _logEvent(self, server, log_message, *, filename=None, color=None, title=None, thumbnail=None):
        ...
        try:
            ...
            if log_back == log_message:
                ...
                footer = datetime.utcnow().strftime("%b %d %Y - %I:%M %p") + \
                    " UTC - Note: Backticks --> Single Quotes"
            if self.wrap:
                # Wraps the message to lines no longer than 70 chars
                log_message = textwrap.fill(
                    log_message, replace_whitespace=False)
            ...
            if filename:
                await logChan.send(file=discord.File(filename))
        except:
    ...
    async def clean(self, ctx, messages=None, *, chan: discord.TextChannel = None):
        ...
        msg = 'Messages cleaned by {}#{} in {} - #{}\n\n'.format(
            ctx.message.author.name, ctx.message.author.discriminator, ctx.guild.name, ctx.channel.name) + msg
        ...
        if self.shouldLog('message.delete', ctx.guild):
            # We're logging
            logmess = '{}#{} cleaned in #{}'.format(
                ctx.author.name, ctx.author.discriminator, chan.name)
            pfpurl = ctx.author.avatar_url if len(
    ...
    async def heartbeat(self, ctx):
        ...
        print("Message:\n{}".format(message)
              if message else "No message returned.")
