...
class VoteKick(commands.Cog):
    ...
    async def checkVotes(self):
        while not self.bot.is_closed():
            ...
            for guild in self.bot.guilds:
                expire_time = self.settings.getServerStat(
                    guild, "VotesResetTime")
                vote_mute = self.settings.getServerStat(guild, "VotesToMute")
                vote_mention = self.settings.getServerStat(
                    guild, "VotesToMention")
                if expire_time == 0:
                    # Never expire
                    continue
    ...
    async def setvkmention(self, ctx, *, user_or_role=None):
        ...
        if check_role:
            self.settings.setServerStat(
                ctx.guild, "VoteKickMention", check_role.id)
        ...
        if check_user:
            self.settings.setServerStat(
                ctx.guild, "VoteKickMention", check_user.id)
    ...
    async def vktomute(self, ctx, *, number_of_votes=None):
        ...
        if number_of_votes == 0:
            ...
            self.settings.setServerStat(
                ctx.guild, "VotesToMute", number_of_votes)
    ...
    async def vktomention(self, ctx, *, number_of_votes=None):
        ...
        if number_of_votes == 0:
            ...
            self.settings.setServerStat(
                ctx.guild, "VotesToMention", number_of_votes)
    ...
    async def setvkchannel(self, ctx, *, channel=None):
        ...
        if check_channel:
            self.settings.setServerStat(
                ctx.guild, "VoteKickChannel", check_channel.id)
    ...
    async def vk(self, ctx, user=None, *, server=None):
        ...
        server_msg = " in **{}**".format(Nullify.escape_all(guild.name)
                                         ) if guild != ctx.guild else ""
        ...
        for member in vote_list:
            if member["ID"] == check_user.id:
                ...
                member["Kicks"].append(
                    {"ID": ctx.author.id, "Added": time.time()})
                # Update the array
                self.settings.setServerStat(guild, "VoteKickArray", vote_list)
    ...
    async def vkclear(self, ctx, *, user=None):
        ...
        for member in vote_list:
            if member["ID"] == check_user.id:
                ...
                self.settings.setServerStat(
                    ctx.guild, "VoteKickArray", vote_list)
