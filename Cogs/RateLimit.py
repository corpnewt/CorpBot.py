...
class RateLimit(commands.Cog):
    ...
    def __init__(self, bot, settings):
        ...
        # 5 seconds between commands - placeholder, overridden by settings
        self.commandCooldown = 5
    ...
    async def message(self, message):
        ...
        # Get current delay
        currDelay = self.settings.getGlobalStat(
            "CommandCooldown", self.commandCooldown)

        # Check if we can run commands
        try:
            lastTime = int(self.settings.getUserStat(
                message.author, message.guild, "LastCommand"))
        except:
    ...
    async def oncommand(self, ctx):
        ...
        self.settings.setUserStat(
            ctx.message.author, ctx.message.guild, "LastCommand", int(time.time()))
