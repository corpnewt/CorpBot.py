...
class Help(commands.Cog):
    ...
    def _get_prefix(self, ctx):
        ...
        bot_member = ctx.guild.get_member(
            self.bot.user.id) if ctx.guild else ctx.bot.user
    ...
    async def _get_info(self, ctx, com=None):
        ...
        footer = "\nType `{}help command` for more info on a command. \n".format(
            prefix)
        footer += "You can also type `{}help category` for more info on a category.".format(
            prefix)
        # Get settings - and check them if they exist
        settings = self.bot.get_cog("Settings")
        disabled_list = settings.getServerStat(
            ctx.guild, "DisabledCommands", []) if settings and ctx.guild else []
        if com:
            ...
            if the_cog:
                ...
                for e in self.bot.extensions:
                    ...
                    if not self._is_submodule(b_ext.__name__, the_cog.__module__):
                        continue
                    # It's a submodule
                    embed_list = {
                        "title": "{} Cog - {}.py Extension". format(com, e[5:]), "fields": []}
                    break
                for command in sorted(the_cog.get_commands(), key=lambda x: x.name):
                    ...
                    name = "{}{} {}{}".format(
                        ...
                        command.name,
                        command.signature,
                        "" if not len(command.aliases) else " (AKA: {})".format(
                            ", ".join(command.aliases))
                    )
                    if command.name in disabled_list:
                        name = "~~" + name + "~~ (Disabled)"
                    embed_list["fields"].append(
                        {"name": name, "value": "`└─ " + command_help + "`", "inline": False})
            ...
            if the_com:
                for e in self.bot.extensions:
                    ...
                    if self._is_submodule(b_ext.__name__, the_com.cog.__module__):
                        # It's a submodule
                        embed_list = {
                            "title": "{} Cog - {}.py Extension".format(the_com.cog_name, e[5:]), "fields": []}
                        break
                name = "**{}{} {}{}**".format(
                    prefix,
                    the_com.name,
                    the_com.signature,
                    "" if not len(the_com.aliases) else " (AKA: {})".format(
                        ", ".join(the_com.aliases))
                )
                embed_list["description"] = "{}\n```\n{}\n```".format(
                    "~~"+name +
                    "~~ (Disabled)" if the_com.name in disabled_list else name,
                    the_com.help
                )
                return embed_list
            return None
    ...
    async def dumphelp(self, ctx, tab_indent_count=None):
        ...
        for cog in sorted(self.bot.cogs):
            ...
            cog_commands = [c for c in sorted(
                the_cog.get_commands(), key=lambda x:x.name) if not c.hidden]
            if not len(cog_commands):
                continue
            cog_count = "{:,} command{}".format(
                len(cog_commands), "" if len(cog_commands) == 1 else "s")
    ...
    async def dumpmarkdown(self, ctx):
        ...
        for cog in sorted(self.bot.cogs):
            ...
            cog_string += "\n"
            msg += cog_string
        msg = ", ".join(["[{}](#{})".format(x, x.lower())
                        for x in sorted(cog_list)])+"\n\n"+msg
        # Encode to binary
        # Trim the trailing newlines
        msg = msg.rstrip().encode("utf-8")
        with open(serverFile, "wb") as myfile:
    ...
    async def help(self, ctx, *, command=None):
        ...
        if result == None:
            ...
            m.description = "No exact Cog or command matches for \"{}\".".format(
                command)
            if len(cog_match):
                cog_mess = "\n".join(["`└─ {}`".format(x["Item"])
                                     for x in cog_match])
            ...
            m.footer = {"text": "Cog and command names are case-sensitive.",
                        "icon_url": self.bot.user.avatar_url}
            return await m.send(ctx)
        result["color"] = ctx.author
        bot_user = ctx.guild.get_member(
            self.bot.user.id) if ctx.guild else self.bot.user
        desc = "```\nGet more info with \"{}help Cog_or_command\".\nCog and command names are case-sensitive.\n\n{}: {}```".format(
            self._get_prefix(ctx), bot_user.display_name, self.bot.description)
