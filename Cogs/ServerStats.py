...
class ServerStats(commands.Cog):
    ...
    async def message(self, message):
        ...
        if not message.author.id == self.bot.user.id:
            ...
            messages = int(self.settings.getServerStat(
        ...
        if not len(bot_list):
            ...
            for y, x in enumerate(bot_list, 1):
                if y > 20:
                    break
                last = y
                bot_text_list.append({"name": "{}#{} ({})".format(
                    x.name, x.discriminator, x.id), "value": x.mention, "inline": False})
            header = "__**Showing {} of {} bot{}:**__".format(
                last, len(bot_list), "" if len(bot_list) == 1 else "s")
    ...
    async def serverinfo(self, ctx, *, guild_name=None):
        ...
        local_time = UserTime.getUserTime(
            ctx.author, self.settings, guild.created_at)
        ...
        server_embed.add_field(name="Members ({:,} total)".format(
            len(guild.members)), value=user_string, inline=True)
        server_embed.add_field(name="Roles", value=str(
            len(guild.roles)), inline=True)
        ...
        server_embed.add_field(name="Default Role",
                               value=guild.default_role, inline=True)
        server_embed.add_field(
            name="Owner", value=guild.owner.name + "#" + guild.owner.discriminator, inline=True)
        server_embed.add_field(
            name="AFK Channel", value=guild.afk_channel, inline=True)
        ...
        server_embed.add_field(name="Voice Region",
                               value=guild.region, inline=True)
        server_embed.add_field(name="Considered Large",
                               value=guild.large, inline=True)
        server_embed.add_field(
            name="Shard ID", value="{}/{}".format(guild.shard_id+1, self.bot.shard_count), inline=True)
        server_embed.add_field(name="Nitro Boosts", value="{} (level {})".format(
        ...
        joinedList = sorted(
            joinedList, key=lambda x: x["Joined"].timestamp() if x["Joined"] != None else -1)
        popList = sorted(popList, key=lambda x: x['Population'], reverse=True)

        check_item = {"ID": guild.id, "Joined": guild.me.joined_at}
        total = len(joinedList)
        position = joinedList.index(check_item) + 1
        server_embed.add_field(name="Join Position", value="{:,} of {:,}".format(
        ...
        server_embed.add_field(name="Population Rank", value="{:,} of {:,}".format(
            position, total), inline=True)
        ...
        if disabledemojis:
            emojifields.append("{:,} unavailable".format(
                disabledemojis))  # Add the disabled if any

        server_embed.set_thumbnail(url=guild.icon_url if len(
            guild.icon_url) else ctx.author.default_avatar_url)
        ...
        for i, e in enumerate(emojifields):
            name = "Disabled Emojis" if e.lower().endswith("unavailable") else "Twitch Emojis" if e.lower(
            ).endswith("managed") else "Emojis ({} of {})".format(i+1, len(emojifields))
        ...
        await Message.Embed(
            ...
            fields=[
                ...
                {"name": "Total", "value": "└─ {:,}/{:,} online ({:,g}%)".format(
                    ...
                    len(users)+len(bots),
                    round(((len(users_online)+len(bots_online)) /
                          (len(users)+len(bots)))*100, 2)
                ), "inline": False}
            ],
    ...
    async def joinedatpos(self, ctx, *, position):
        ...
        joinedList = [{"member": mem, "joined": mem.joined_at}
        ...
        msg = "*{}* joined at position **{:,}**.".format(
            DisplayName.name(join["member"]), position+1)
        await ctx.send(msg)
            ...
            our_list.append(
                {
                    ...
                    # UserTime.getUserTime(ctx.author,self.settings,member.joined_at,force=offset)["vanity"],
    ...
    async def recentjoins(self, ctx):
        ...
        our_list = sorted(our_list, key=lambda x: x["date"].timestamp(
        ) if x["date"] != None else -1, reverse=True)
            ...
            our_list.append(
                {
                    ...
                    # UserTime.getUserTime(ctx.author,self.settings,bot.joined_at,force=offset)["vanity"],
    ...
    async def messages(self, ctx):
        ...
        messages = int(self.settings.getServerStat(
            ctx.message.guild, "TotalMessages"))
        messages -= 1
        self.settings.setServerStat(
            ctx.message.guild, "TotalMessages", messages)
        if messages == None:
            messages = 0
        if messages == 1:
    ...
    async def allmessages(self, ctx):
        ...
        messages = 0
        for guild in self.bot.guilds:
            temp = 0 if self.settings.getServerStat(
                guild, "TotalMessages") is None else self.settings.getServerStat(guild, "TotalMessages")
            messages += int(temp)
        messages -= 1
        if messages == 1:
