...
class Search(commands.Cog):
    ...
    async def searchsite(self, ctx, category_name=None, *, query=None):
        ...
        search_url = "https://corpnewt.com/api/search?term={}&in=titlesposts&categories[]={}&searchChildren=true&showAs=posts".format(
            query, category["cid"])
        ...
        for post in posts:
            if ctr < limit:
                ...
                result_string += '__{}__\n<https://corpnewt.com/topic/{}>\n\n'.format(
                    post["topic"]["title"], post["topic"]["slug"])
    ...
    async def convert(self, ctx, *, amount=None, frm=None, to=None):
        ...
        try:
            ...
            return await ctx.send("Something went wrong!  The API I use may be down :(")
        ...
        if amount == None:
            ...
            for l in r.get("results", {}):
                ...
                curr_list.append(
                    "{} - {}".format(r["results"][l]["id"], r["results"][l]["currencyName"]))
            if len(curr_list):
        ...
        for val in vals:
            if all(x in "+-0123456789." for x in val if not x == ",") and num is None:
                ...
                last = None
            if len(conv) >= 2 and num != None:
                break  # We have enough info - bail
        if num is None:
            num = 1
        if len(conv) < 2:
