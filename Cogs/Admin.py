...
class Admin(commands.Cog):
    ...
    async def message(self, message):
        ...
        isMute = self.settings.getUserStat(
            message.author, message.guild, "Muted")
        ...
        if isMute:
            ignore = True
            delete = True
            checkTime = self.settings.getUserStat(
            ...
            if checkTime and currentTime >= checkTime:
                ...
                self.settings.setUserStat(
                    message.author, message.guild, "Cooldown", None)
                self.settings.setUserStat(
                    message.author, message.guild, "Muted", False)
    ...
    async def defaultchannel(self, ctx):
        ...
        if targetChan == None:
            ...
            if default == None:
                ...
                msg = "The default channel is the server's original default: {}".format(
                    default.mention)
        else:
            # We have a custom channel
            msg = "The default channel is set to **{}**.".format(
                targetChan.mention)
    ...
    async def setdefaultchannel(self, ctx, *, channel: discord.TextChannel = None):
        ...
        if channel == None:
            self.settings.setServerStat(
                ctx.message.guild, "DefaultChannel", "")
            if default == None:
                msg = 'Default channel has been *removed completely*.'
            else:
                msg = 'Default channel has been returned to the server\'s original:  **{}**.'.format(
                    default.mention)
        ...
        self.settings.setServerStat(
            ctx.message.guild, "DefaultChannel", channel.id)
    ...
    async def setmadlibschannel(self, ctx, *, channel: discord.TextChannel = None):
        ...
        if channel == None:
            self.settings.setServerStat(
                ctx.message.guild, "MadLibsChannel", "")
        ...
        # If we made it this far - then we can add it
        self.settings.setServerStat(
            ctx.message.guild, "MadLibsChannel", channel.id)
    ...
    async def xpreservelimit(self, ctx, *, limit=None):
        ...
        if limit == None:
            ...
            server_lim = self.settings.getServerStat(
                ctx.guild, "XPReserveLimit")
            if server_lim == None:
    ...
    async def addxprole(self, ctx, *, role=None, xp: int = None):
        ...
        for aRole in promoArray:
            ...
            if str(aRole['ID']) == str(role.id):
                ...
                aRole['XP'] = xp
                msg = '**{}** updated!  Required xp:  *{:,}*'.format(
                    role.name, xp)
                msg = Utils.suppressed(ctx, msg)
                await channel.send(msg)
                return
        ...
        msg = '**{}** added to list.  Required xp: *{:,}*'.format(
            role.name, xp)
        msg = Utils.suppressed(ctx, msg)
        await channel.send(msg)
        return
        ...
        if type(role) is str:
            ...
            for aRole in promoArray:
                ...
                if aRole['Name'].lower() == role.lower() or str(aRole["ID"]) == str(role):
                    ...
                    self.settings.setServerStat(
                        server, "PromotionArray", promoArray)
    ...
    async def setxprole(self, ctx, *, role: str = None):
        ...
        if role == None:
            self.settings.setServerStat(
                ctx.message.guild, "RequiredXPRole", "")
        ...
        # If we made it this far - then we can add it
        self.settings.setServerStat(
            ctx.message.guild, "RequiredXPRole", role.id)

        msg = 'Role required to give xp, gamble, or feed the bot set to **{}**.'.format(
            role.name)
        msg = Utils.suppressed(ctx, msg)
        await ctx.message.channel.send(msg)
            ...
            for arole in ctx.message.guild.roles:
                if str(arole.id) == str(role):
                    ...
                    if arole.name[:1].lower() in vowels:
                        msg = 'You need to be an **{}** to *give xp*, *gamble*, or *feed* the bot.'.format(
                            arole.name)
                    else:
                        msg = 'You need to be a **{}** to *give xp*, *gamble*, or *feed* the bot.'.format(
                            arole.name)
            if not found:
    ...
    async def setstoprole(self, ctx, *, role: str = None):
        ...
        if role == None:
            self.settings.setServerStat(
                ctx.message.guild, "RequiredStopRole", "")
        ...
        # If we made it this far - then we can add it
        self.settings.setServerStat(
            ctx.message.guild, "RequiredStopRole", role.id)

        msg = 'Role required to stop the music player set to **{}**.'.format(
            role.name)
        msg = Utils.suppressed(ctx, msg)
        await ctx.message.channel.send(msg)
    ...
    async def stoprole(self, ctx):
        ...
        role = self.settings.getServerStat(
            ctx.message.guild, "RequiredStopRole")
        if role == None or role == "":
            ...
            for arole in ctx.message.guild.roles:
                if str(arole.id) == str(role):
                    ...
                    vowels = "aeiou"
                    if arole.name[:1].lower() in vowels:
                        msg = 'You need to be an **{}** to use `$stop`.'.format(
                            arole.name)
                    else:
                        msg = 'You need to be a **{}** to use `$stop`.'.format(
                            arole.name)
    ...
    async def setlinkrole(self, ctx, *, role: str = None):
        ...
        if role == None:
            self.settings.setServerStat(
                ctx.message.guild, "RequiredLinkRole", "")
        ...
        # If we made it this far - then we can add it
        self.settings.setServerStat(
            ctx.message.guild, "RequiredLinkRole", role.id)

        msg = 'Role required for add/remove links set to **{}**.'.format(
            role.name)
        msg = Utils.suppressed(ctx, msg)
        await ctx.message.channel.send(msg)
    ...
    async def sethackrole(self, ctx, *, role: str = None):
        ...
        if role == None:
            self.settings.setServerStat(
                ctx.message.guild, "RequiredHackRole", "")
        ...
        # If we made it this far - then we can add it
        self.settings.setServerStat(
            ctx.message.guild, "RequiredHackRole", role.id)

        msg = 'Role required for add/remove hacks set to **{}**.'.format(
            role.name)
        msg = Utils.suppressed(ctx, msg)
        await ctx.message.channel.send(msg)
    ...
    async def settagrole(self, ctx, *, role: str = None):
        ...
        if role == None:
            self.settings.setServerStat(
                ctx.message.guild, "RequiredTagRole", "")
        ...
        # If we made it this far - then we can add it
        self.settings.setServerStat(
            ctx.message.guild, "RequiredTagRole", role.id)

        msg = 'Role required for add/remove tags set to **{}**.'.format(
            role.name)
        msg = Utils.suppressed(ctx, msg)
        await ctx.message.channel.send(msg)
    ...
    async def rawrules(self, ctx):
        ...
        msg = "*{}* Rules (Raw Markdown):\n{}".format(
            Utils.suppressed(ctx, ctx.guild.name), rules)
        await ctx.channel.send(msg)
    ...
    async def addadmin(self, ctx, *, role: str = None):
        ...
        self.settings.setServerStat(
            ctx.message.guild, "AdminArray", promoArray)
    ...
    async def settopic(self, ctx, channel=None, *, message=""):
        ...
        resolved_channel = DisplayName.channelForName(
            channel, ctx.guild, "text")
        if not resolved_channel:
            # Not a channel - assume it's part of the message
            message = channel + message
            resolved_channel = ctx.channel
        try:
            await resolved_channel.edit(topic=message if message else None)
        except:
            return await ctx.send("Something went wrong when changing the channel topic :(")
    ...
    async def slowmode(self, ctx, channel=None, seconds=None):
        ...
        if seconds == None:  # Print the slow mode
            ...
            return await ctx.send("Slow mode is {} in {}!".format(
                "enabled with a delay of {:,} second{}".format(
                    sm, "" if sm == 1 else "s") if sm > 0 else "" if sm > 0 else "disabled",
                resolved_channel.mention,
            ))
        try:
            seconds = int(seconds)
        except:
            return await ctx.send("Invalid seconds value passed!")
        try:
            await resolved_channel.edit(slowmode_delay=0 if seconds <= 0 else seconds)
        except:
            return await ctx.send("Something went wrong when updating slow mode :(")
        return await ctx.send("Slow mode is {} in {}!".format(
            "enabled with a delay of {:,} second{}".format(
                seconds, "" if seconds == 1 else "s") if seconds > 0 else "disabled",
            resolved_channel.mention
        ))
