...
class XpStack(commands.Cog):
    ...
    async def setxpcount(self, ctx, count=None):
        ...
        if count == None:
            ...
            self.settings.setServerStat(
                ctx.guild, "XP Count", self.xp_save_count)
    ...
    async def checkxp(self, ctx):
        ...
        if page_total > 1:
            help_embed.title = title + \
                " (Page {:,} of {:,})".format(page_count, page_total)
        for i in range(len(xp_array)):
            ...
            f_value = (f_value[:1021] +
                       "...") if len(f_value) > 1024 else f_value
            ...
            if len(help_embed.fields) >= 25:
                if page_total == page_count:
                    help_embed.set_footer(
                        text="Requested by " + str(ctx.author))
    ...
    async def on_xp(self, to_user, from_user, amount):
        ...
        xp_transaction = {"To": to_name, "From": f_name, "Time": datetime.today(
        ).strftime("%Y-%m-%d %H.%M"), "Amount": amount}
