...
def clean(string, deaden_links=False, ctx=None):
    ...
    if deaden_links:
        ...
        for m in matches:
            ...
            string = string[0:m.start()+i_adjust] + match + \
                string[i_adjust+m.end():]
...
def resolve_mentions(string, ctx=None, escape=True, show_mentions=True, channel_mentions=False):
    ...
    if guild:
        ...
        for m in matches:
            try:
                id_match = int(d.search(m.group(0)).group(0))
            except:
                continue
            check_func = guild.get_role if "@&" in m.group(
                0) else guild.get_channel if "#" in m.group(0) else guild.get_member
            check_entry = check_func(id_match)
            if not check_entry:
                continue
            # Let's replace the indices in the original string - but then also update our adjusted index
            name = check_entry.display_name if hasattr(
                check_entry, "display_name") else check_entry.name
            if escape:
                name = escape_all(name)
            # Add the @ prefix if we're showing mentions
            if show_mentions:
                name = ("#" if "#" in m.group(0) else "@")+name
            string = string[0:m.start()+i_adjust] + name + \
                string[i_adjust+m.end():]
            i_adjust += len(name)-len(m.group(0))
    # Catch any missing mentions as needed
    return escape_all(string, markdown=False) if escape else string
