...
class WatchURL(commands.Cog):
    ...
    async def on_message(self, message):
        ...
        url_list = self.settings.getServerStat(
            message.guild, "URLWatchList", [])
        if not url_list:
            return
        # Check if we have a link
        matches = re.finditer(self.regex, message.content)
        valid_matches = list(set([match.group(
            0) for match in matches for x in url_list if x.lower() in match.group(0).lower()]))
        if not len(valid_matches):
            return
        matched = self.settings.getServerStat(
            message.guild, "URLWatchListMatches", [])
        for match in valid_matches:
            matched.insert(0, {"from": message.author.id, "url": match,
        ...
        self.settings.setServerStat(
            message.guild, "URLWatchListMatches", matched)
    ...
    async def listwatchurls(self, ctx):
        ...
        msg = "\n".join(url_list) if len(
            url_list) else "There are no URLs being watched for."
    ...
    async def lasturls(self, ctx):
        ...
        url_matches = self.settings.getServerStat(
            ctx.guild, "URLWatchListMatches", [])
        if not url_matches:
        ...
        entries = [{"name": "{}. {}".format(y+1, x["url"]), "value":"└─ From <@!{}> - [Link]({})".format(
            x["from"], x["link"])} for y, x in enumerate(url_matches)]
