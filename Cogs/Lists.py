...
class Lists(commands.Cog):
    ...
    def _ensure_keys(self, ctx, l_list, item_list, l_key):
        ...
        valid_keys = (l_key, "Name", "Created", "CreatedID",
                      "CreatedBy", "Updated", "UpdatedID", "UpdatedBy")
        to_remove = []
        changed = False
    ...
    async def _get_item(self, ctx, name, l_role="RequiredLinkRole", l_list="Links", l_name="Link", l_key="URL", raw=False):
        ...
        if not name:
            msg = 'Usage: `{}{}[[name]] "[[[name]] name]"`'.format(
                ctx.prefix, "raw" if raw else "").replace("[[name]]", l_name.lower())
        ...
        for i in self.alt_lists:
            ...
            if any(x["Name"].lower() == name.lower() for x in check_list):
                ...
                other_names.append("{}{} {}".format(
                    ctx.prefix, i["command"], Nullify.escape_all(name)))

        if not itemList or itemList == []:
            no_items = 'No [[name]]s in list!  You can add some with the `{}add[[name]] "[[[name]] name]" [[[key]]]` command!'.format(
                ctx.prefix).replace("[[name]]", l_name.lower()).replace("[[key]]", l_key.lower())
        ...
        for item in itemList:
            if item['Name'].lower() == name.lower():
                msg = '**{}:**\n{}'.format(Nullify.escape_all(
                    item['Name']), discord.utils.escape_markdown(item[l_key]) if raw else item[l_key])
                return await ctx.send(Utils.suppressed(ctx, msg))

        not_found = '{} not found in {} list!'.format(
            Nullify.escape_all(name), l_name.lower())
    ...
    async def _add_item(self, ctx, name, value, l_role="RequiredLinkRole", l_list="Links", l_name="Link", l_key="URL"):
        ...
        item = next(
            (x for x in itemList if x["Name"].lower() == name.lower()), None)
        if item:
            ...
            itemList.append({"Name": name, l_key: value, "CreatedBy": DisplayName.name(
                ctx.author), "CreatedID": ctx.author.id, "Created": currentTime})
            msg = '{} added to {} list!'.format(
                Nullify.escape_all(name), l_name.lower())
    ...
    async def _item_info(self, ctx, name, l_role="RequiredLinkRole", l_list="Links", l_name="Link", l_key="URL"):
        ...
        # Get the info
        created_by = DisplayName.memberForID(
            item.get("CreatedID", 0), ctx.guild)
        created_by = DisplayName.name(
            created_by) if created_by else item.get("CreatedBy", "`UNKNOWN`")
        msg += "Created by: {}\n".format(created_by)
        created = item.get("Created", None)
        if created:
            msg += "Created: {} ago\n".format(
                ReadableTime.getReadableTimeBetween(created, current_time, True))
        if item.get("Updated", None):
            updated_by = DisplayName.memberForID(
                item.get("UpdatedID", 0), ctx.guild)
            updated_by = DisplayName.name(
                updated_by) if updated_by else item.get("UpdatedBy", "`UNKNOWN`")
            msg += "Updated by: {}\n".format(updated_by)
            updated = item.get("Updated", None)
            if created:
                msg += "Updated: {} ago\n".format(
                    ReadableTime.getReadableTimeBetween(updated, current_time, True))
        return await ctx.send(msg)
    ...
    async def _list_items(self, ctx, command, l_role="RequiredLinkRole", l_list="Links", l_name="Link", l_key="URL", raw=False):
        ...
        if not itemList or itemList == []:
            ...
            return await ctx.send(msg)
        # Sort by link name
        items = [{"name": "{}. {}".format(i, x["Name"]), "value":Utils.truncate_string(
            x[l_key])} for i, x in enumerate(sorted(itemList, key=lambda x:x["Name"].lower()), start=1)]
    ...
    async def _get_role(self, ctx, l_role="RequiredLinkRole", l_list="Links", l_name="Link", l_key="URL"):
        ...
        if role == None or role == "":
            msg = '**Only Admins** can add and remove {}s.'.format(
                l_name.lower())
            return await ctx.send(msg)
    ...
    async def parts(self, ctx, *, member=None):
        ...
        if not parts or parts == "":
            msg = '*{}* has not added their parts yet!  ~~They can add them with the `{}setparts [parts text]` command!~~ DEPRECATED - Use `{}newhw` instead.'.format(
                DisplayName.name(member), ctx.prefix, ctx.prefix)
            return await ctx.send(msg)
        msg = '***{}\'s*** **Parts (DEPRECATED - Use {}hw instead):**\n\n{}'.format(
            DisplayName.name(member), ctx.prefix, parts)
    ...
    async def setparts(self, ctx, *, parts: str = None):
        ...
        msg = '*{}\'s* parts have been set to (DEPRECATED - Use {}newhw instead):\n{}'.format(
            DisplayName.name(ctx.author), ctx.prefix, parts)
        await ctx.send(Utils.suppressed(ctx, msg))
    ...
    async def online(self, ctx):
        ...
        await Message.Embed(
            title="Member Stats",
            description="Current member information for {}".format(
                ctx.guild.name),
            fields=[
                {"name": "Members", "value": "└─ {:,}/{:,} online ({:,g}%)".format(
                    membersOnline, members, round((membersOnline/members)*100, 2)), "inline": False},
                {"name": "Bots", "value": "└─ {:,}/{:,} online ({:,g}%)".format(
                    botsOnline, bots, round((botsOnline/bots)*100, 2)), "inline": False},
                {"name": "Total", "value": "└─ {:,}/{:,} online ({:,g}%)".format(membersOnline + botsOnline, len(
                    ctx.guild.members), round(((membersOnline + botsOnline)/len(ctx.guild.members))*100, 2)), "inline": False}
            ],
    ...
    async def lastonline(self, ctx, *, member=None):
        ...
        if not member.status == discord.Status.offline:
            ...
            lastOnline = self.settings.getUserStat(
                member, ctx.guild, "LastOnline")
            if lastOnline == "Unknown":
                self.settings.setUserStat(
                    member, ctx.guild, "LastOnline", None)
                lastOnline = None
            if lastOnline:
                currentTime = int(time.time())
                timeString = ReadableTime.getReadableTimeBetween(
                    int(lastOnline), currentTime, True)
                msg = 'The last time I saw *{}* was *{} ago*.'.format(
                    name, timeString)
            else:
                msg = 'I don\'t know when *{}* was last online.  Sorry.'.format(
                    name)
        await ctx.send(msg)
