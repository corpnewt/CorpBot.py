...
class OfflineUser(commands.Cog):
    ...
    async def on_message(self, message):
        ...
        if not len(message.mentions):
            return
        name_list = [DisplayName.name(
            x) for x in message.mentions if x.status is discord.Status.offline]
        if not len(name_list):
            # No one was offline
            return
        if len(name_list) == 1:
            msg = "{}, it looks like {} is offline - pm them if urgent.".format(
                ctx.author.mention, name_list[0])
        else:
            msg = "{}, it looks like the following users are offline - pm them if urgent:\n\n{}".format(
                ctx.author.mention, ", ".join(name_list))
