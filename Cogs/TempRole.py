...
class TempRole(commands.Cog):
        ...
        for server in self.bot.guilds:
            for member in server.members:
                if not self.is_current:
                    ...
                    return
                temp_roles = self.settings.getUserStat(
                    member, server, "TempRoles", [])
                if len(temp_roles):
                    # We have a list
                    remove_temps = []
                    for temp_role in temp_roles:
                        ...
                        self.bot.loop.create_task(
                            self.check_temp_roles(member, temp_role))
    ...
    async def check_temp_roles(self, member, temp_role):
        if not self.is_current:
            return  # Bail if we're not current

        # Get a handle on the current temp role
        temp_roles = self.settings.getUserStat(
            member, member.guild, "TempRoles", [])
        found_role = next(
            (x for x in temp_roles if x["ID"] == temp_role["ID"]), None)
        if not found_role:
            return  # Didn't find it - bail
        ...
        self.settings.setUserStat(member, member.guild, "TempRoles", [
                                  x for x in temp_roles if not x["ID"] == found_role["ID"]])
    ...
    async def autotemp(self, ctx, *, role=None):
        ...
        msg = "**{}** is now the default temp role - will be active for *{}*.".format(
            Nullify.escape_all(role.name), ReadableTime.getReadableTimeBetween(0, role_time * 60))
        await ctx.send(msg)
    ...
    async def getautotemp(self, ctx):
        ...
        msg = "**{}** is the default temp role - will be active for *{}*.".format(
            Nullify.escape_all(temp_role.name), ReadableTime.getReadableTimeBetween(0, role_time * 60))
        await ctx.send(msg)
    ...
    async def temptime(self, ctx, *, minutes=None):
        ...
        msg = "Temp role will last *{}*.".format(
            ReadableTime.getReadableTimeBetween(0, minutes*60))
        await ctx.send(msg)
    ...
    async def hastemp(self, ctx, *, member=None):
        ...
        if member == None:
            ...
            if not member:
                msg = 'I couldn\'t find *{}*...'.format(
                    Nullify.escape_all(member_name))
                await ctx.send(msg)
                return
        # Got the member - let's check for roles
        temp_roles = self.settings.getUserStat(
            member, ctx.guild, "TempRoles", [])
            ...
            for role in ctx.guild.roles:
                if str(role.id) == str(arole['ID']):
                    ...
                    if not "AddedBy" in arole:
                        ...
                        add_user = DisplayName.name(
                            DisplayName.memberForID(arole["AddedBy"], ctx.guild))
                        if not add_user:
                            add_user = str(arole["AddedBy"])
                        added = "by {}".format(add_user)
                    roleText = '{}**{}** - added {} - *{}* remain\n'.format(roleText, Nullify.escape_all(
                        role.name), added, ReadableTime.getReadableTimeBetween(0, timeleft))
            if not foundRole:
    ...
    async def removetemprole(self, ctx, *, role: str = None):
        ...
        if role.id in [int(x["ID"]) for x in promoArray]:
            ...
            msg = '**{}** removed successfully.'.format(
                Nullify.escape_all(role.name))
            self.settings.setServerStat(ctx.guild, "TempRoleList", [
                                        x for x in promoArray if role.id != int(x["ID"])])
            await ctx.send(msg)
            return
    ...
    async def untemp(self, ctx, member=None, role=None):
        ...
        user_roles = self.settings.getUserStat(
            member_from_name, ctx.guild, "TempRoles", [])
        for r in user_roles:
            if int(r["ID"]) == role_from_name.id:
                user_roles.remove(r)
                self.settings.setUserStat(
                    member_from_name, ctx.guild, "TempRoles", user_roles)
                break
    ...
    async def temp(self, ctx, member=None, role=None, *, cooldown=None):
        ...
        if member == None or role == None:
            msg = 'Usage: `{}temp "[member]" "[role]" [cooldown]`'.format(
                ctx.prefix)
        ...
        if Utils.is_bot_admin(ctx, member_from_name):
            return await ctx.send("You can't apply temp roles to other admins or bot-admins.")
        ...
        for r in user_roles:
            if int(r["ID"]) == role_from_name.id:
                ...
                r["Cooldown"] = cooldown + \
                    int(time.time()) if cooldown != None else cooldown
