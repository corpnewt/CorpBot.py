...
class Settings:
	...
	def __init__(self, bot, file : str = None):
		...
		self.mongoDB = self.mongoClient.corpbot
		...
		self.defaultMember = {
			"Name" 			: "",
			"DisplayName" 	: "",
			"XP" 			: self.defaultServer["DefaultXP"],
			"XPReserve" 	: self.defaultServer["DefaultXPReserve"],
			"DiscordId" 	: "",
			"Discriminator" : "",
			"Parts"			: "",
			"Muted"			: "No",
	...
	def checkServer(self, server):
		...
		servers = self.mongoDB.servers
		
		# Find the server in mongodb 
		newServer = {"Name": server.name, "DiscordId": server.id}
		server = servers.find_one(newServer)
		# Check if server is found
		if server == None:
			#Create new server and get MongoDb Id after insert
			result = servers.insert_one(newServer).inserted_id
	...
	def checkUser(self, user, server):
		...
		members = self.mongoDB.members
		
		# Find the server in mongodb 
		findMember = {"DiscordId": user.id, "ServerId": server.id}
		mongoMember = members.find_one(findMember)
		# Check if server is found
		if mongoMember == None:
			#Create new member and get MongoDb Id after insert
			newMember = copy.deepcopy(self.defaultMember)
			newMember["Name"] = user.name
			newMember["DisplayName"] = user.display_name
			newMember["DiscordId"] = user.id
			newMember["Discriminator"] = user.discriminator
			newMember["ServerId"] = server.id
			result = members.insert_one(newMember).inserted_id
			return newMember
		else:
			return mongoMember
	...
	def getUserStat(self, user, server, stat):
		...
		print('getting user stat')
		user = self.checkUser(user, server)
		# Check for our username
		if stat in user:
			print(user[stat])
			return user[stat]
	...
	def getServerStat(self, server, stat):
		stats = self.mongoDB.server_stats
		
		# Find the server in mongodb 
		query = {"ServerId": server.id, "StatName": stat}
		mongoServerStat = stats.find_one(query)
		result = self.defaultServer[stat]
		if mongoServerStat != None:
			result = mongoServerStat["StatValue"]
		return result
		# # Make sure our server exists in the list
		# self.checkServer(server)
