...
class Discogs(commands.Cog):
    ...
    async def discog(self, url, ctx):
        ...
        try:
            r = await DL.async_json(url)
        except:
            return await ctx.send("Something went wrong!  The API I use may be down, or maybe too many requests happened too quickly :(")
        ...
        for x in ("genre", "style", "year", "type", "label"):
            if not x in result:
                continue
            fields.append({"name": x.capitalize(), "value": ", ".join(result[x]) if isinstance(
                result[x], list) else str(result[x]).capitalize(), "inline": True})
        if not len(fields):
    ...
    async def discogs(self, ctx, *, search=None):
        ...
        # Get the search results - per_page maxes at 100
        url = "https://api.discogs.com/database/search?{}&per_page=10&token={}".format(
