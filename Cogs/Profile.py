...
class Profile(commands.Cog):
    ...
    async def addprofile(self, ctx, name=None, *, link=None):
        ...
        if item:
            msg = Utils.suppressed(ctx, "{}'s {} profile was updated!".format(
                DisplayName.name(ctx.author), Nullify.escape_all(item["Name"])))
            item["URL"] = link
            item["Updated"] = currentTime
        else:
            itemList.append(
                {"Name": name, "URL": link, "Created": currentTime})
            msg = Utils.suppressed(ctx, "{} added to {}'s profile list!".format(
    ...
    def _get_profile(self, ctx, name=None):
        ...
        for j in range(len(parts)):
            ...
            if mem_from_name:
                ...
                itemList = self.settings.getUserStat(
                    mem_from_name, ctx.guild, "Profiles", [])
                item = next(
                    (x for x in itemList if x["Name"].lower() == name.lower()), None)
                if item:
    ...
    async def _get_profile_reply(self, ctx, name=None, raw=False):
        if not name:
            msg = "Usage: `{}{}profile [member] [profile name]`".format(
                ctx.prefix, "raw" if raw else "")
            return await ctx.send(msg)
        ...
        msg = '*{}\'s {} Profile:*\n\n{}'.format(DisplayName.name(
            member), item['Name'], discord.utils.escape_markdown(item['URL']) if raw else item['URL'])
    ...
    async def _list_profiles(self, ctx, member=None, raw=None):
        ...
        itemText += discord.utils.escape_markdown("\n".join(
            [x["Name"] for x in itemList])) if raw else "\n".join([x["Name"] for x in itemList])
    ...
    async def profileinfo(self, ctx, *, member=None, name=None):
        ...
        msg = '**{}:**\n'.format(item['Name'])
        msg += "Created: {} ago\n".format(ReadableTime.getReadableTimeBetween(item.get(
            "Created", None), current_time, True)) if item.get("Created", None) else "Created: `UNKNOWN`\n"
        if item.get("Updated", None):
            msg += "Updated: {} ago\n".format(
                ReadableTime.getReadableTimeBetween(item["Updated"], current_time, True))
        return await ctx.send(Utils.suppressed(ctx, msg))
