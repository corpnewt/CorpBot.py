        ...
        if xpRAmount > 0:
            ...
            if not xprLimit == None:
                ...
                newxp = self.settings.getUserStat(
                    message.author, server, "XPReserve")
                ...
                if newxp + xpRAmount > xprLimit:
                    ...
                    if newxp < xprLimit:
                        self.settings.setUserStat(
                            message.author, server, "XPReserve", xprLimit)
            if not skip:
                # Bump xp reserve
                self.settings.incrementStat(
                    message.author, server, "XPReserve", xpRAmount)
        ...
        if xpAmount > 0:
            ...
            if not xpLimit == None:
                ...
                if newxp + xpAmount > xpLimit:
                    ...
                    if newxp < xpLimit:
                        self.settings.setUserStat(
                            message.author, server, "XP", xpLimit)
            if not skip:
                # Bump xp
                self.settings.incrementStat(
                    message.author, server, "XP", xpAmount)
