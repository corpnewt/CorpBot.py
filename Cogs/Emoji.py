...
class Emoji(commands.Cog):
    ...
    def _get_emoji_url(self, emoji):
        ...
        emojiparts = emoji.replace("<", "").replace(
            ">", "").split(":") if emoji else []
        if not len(emojiparts) == 3:
            return None
        # Build a custom emoji object
        emoji_obj = discord.PartialEmoji(animated=len(
            emojiparts[0]) > 0, name=emojiparts[1], id=emojiparts[2])
        # Return the url
    ...
    async def addemoji(self, ctx, *, emoji=None, name=None):
        ...
        omitted = " ({} omitted, beyond the limit of {})".format(len(
            emojis_to_add)-self.max_emojis, self.max_emojis) if len(emojis_to_add) > self.max_emojis else ""
        ...
        for emoji_to_add in emojis_to_add[:self.max_emojis]:
            ...
            try:
                new_emoji = await ctx.guild.create_custom_emoji(name=e_name, image=image, roles=None, reason="Added by {}#{}".format(ctx.author.name, ctx.author.discriminator))
            except:
                continue
        ...
        if len(added_emojis):
            msg += "\n\n"
            emoji_text = [
                "{} - `:{}:`".format(self._get_emoji_mention(x), x.name) for x in added_emojis]
            msg += "\n".join(emoji_text)
    ...
    async def emoji(self, ctx, emoji=None):
        ...
        if not f:
            return await ctx.send("I couldn't get that emoji :(")
