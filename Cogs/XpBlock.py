...
class XpBlock(commands.Cog):
    ...
    async def xpblock(self, ctx, *, user_or_role: str = None):
        ...
        promoArray = self.settings.getServerStat(
            ctx.message.guild, "XpBlockArray")
        ...
        self.settings.setServerStat(
            ctx.message.guild, "XpBlockArray", promoArray)
    ...
    async def xpunblockall(self, ctx):
        ...
        xparray = self.settings.getServerStat(
            ctx.message.guild, "XpBlockArray")
    ...
    async def listxpblock(self, ctx):
        ...
        if not len(promoArray):
            roleText = "There are no users or roles xp blocked yet.  Use `{}xpblock [user_or_role]` to add some.".format(
                ctx.prefix)
        ...
        for arole in promoArray:
            ...
            if test:
                # It's a role
                roleText = roleText + \
                    "**{}** (Role), ".format(Nullify.escape_all(test.name))
                continue
            # Didn't find a role or person
            roleText = roleText + \
                "**{}** (removed from server), ".format(arole)
