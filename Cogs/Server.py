...
class Server(commands.Cog):
    ...
    async def getprefix(self, ctx):
        ...
        msg = 'Prefix{}: {}'.format("es are" if len(
            prefix) > 1 else " is", prefixlist)
        await ctx.send(msg)
    ...
    async def leaveserver(self, ctx, *, targetServer=None):
        ...
        guild = next((x for x in self.bot.guilds if str(
            x.id) == str(targetServer)), None)
        if not guild:
            guild = next(
                (x for x in self.bot.guilds if x.name.lower() == targetServer.lower()), None)
        if guild:
            await guild.leave()
            try:
