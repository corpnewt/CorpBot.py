...
class Feed(commands.Cog):
    ...
    async def message(self, message):
        ...
        current_ignore = self.settings.getServerStat(
            message.guild, "IgnoreDeath")
        if current_ignore:
        ...
        if (isKill or hunger >= 100 and hungerLock):
            ignore = not context.command or not context.command.name in [
                "iskill", "resurrect", "hunger", "feed"]
    ...
    async def hunger(self, ctx):
        ...
        overweight = hunger * -1
        if hunger <= -1:
            msg = 'I\'m stuffed ({:,}% overweight)... maybe I should take a break from eating...'.format(
                overweight)
        elif hunger <= -10:
            msg = 'I\'m pudgy ({:,}% overweight)... I may get fat if I keeps going.'.format(
                overweight)
        elif hunger <= -25:
            msg = 'I am, well fat ({:,}% overweight)... Diet time?'.format(
                overweight)
        elif hunger <= -50:
            msg = 'I\'m obese ({:,}% overweight)... Eating is my enemy right now.'.format(
                overweight)
        elif hunger <= -75:
            msg = 'I look fat to an extremely unhealthy degree ({:,}% overweight)... maybe you should think about *my* health?'.format(
                overweight)
        elif hunger <= -100:
            msg = 'I am essentially dead from over-eating ({:,}% overweight).  I hope you\'re happy.'.format(
                overweight)
        elif hunger <= -150:
            msg = 'I *AM* dead from over-eating ({:,}% overweight).  You will have to `{}resurrect` me to get me back.'.format(
                overweight, ctx.prefix)
        elif hunger == 0:
            msg = 'I\'m full ({:,}%).  You are safe.  *For now.*'.format(hunger)
        elif hunger <= 15:
            msg = 'I feel mostly full ({:,}%).  I am appeased.'.format(hunger)
        elif hunger <= 25:
            msg = 'I feel a bit peckish ({:,}%).  A snack is in order.'.format(
                hunger)
        elif hunger <= 50:
            msg = 'I\'m hungry ({:,}%).  Present your offerings.'.format(
                hunger)
        elif hunger <= 75:
        ...
        if isKill and hunger > -150:
            msg = 'I *AM* dead.  Likely from *lack* of care.  You will have to `{}resurrect` me to get me back.'.format(
                overweight, ctx.prefix)
        await ctx.send(msg)
    ...
    async def feed(self, ctx, food: int = None):
        ...
        botAdminAsAdmin = self.settings.getServerStat(
            ctx.guild, "BotAdminAsAdmin")
        adminUnlim = self.settings.getServerStat(ctx.guild, "AdminUnlimited")
        reserveXP = self.settings.getUserStat(
            ctx.author, ctx.guild, "XPReserve")
        ...
        if approve:
            ...
            if decrement:
                self.settings.incrementStat(
                    ctx.author, ctx.guild, "XPReserve", takeReserve)

            if isKill:
                # Bot's dead...
                msg = '*{}* carelessly shoves *{:,} xp* into the carcass of *{}*... maybe resurrect them first next time?'.format(
                    DisplayName.name(ctx.author), food, DisplayName.serverNick(self.bot.user, ctx.guild))
            ...
            if randnum == 1:
                # YOU WON!!
                self.settings.incrementStat(
                    ctx.author, ctx.guild, "XP", int(payout))
            ...
            if hunger <= -150:
                ...
                self.settings.setServerStat(
                    ctx.guild, "KilledBy", ctx.author.id)
                msg = '{}\n\nI am kill...\n\n*{}* did it...'.format(
    ...
    async def kill(self, ctx):
        ...
        requiredRole = self.settings.getServerStat(
            ctx.guild, "RequiredKillRole")
        if requiredRole == "":
            # admin only
    ...
    async def killrole(self, ctx):
        ...
        arole = next(
            (x for x in ctx.guild.roles if str(x.id) == str(role)), None)
        if not arole:
            msg = 'There is no role that matches id: `{}` - consider updating this setting.'.format(
                role)
        msg = 'You need to be a/an **{}** to kill/ressurect the bot.'.format(
            arole.name)
        await ctx.send(Utils.suppressed(ctx, msg))
