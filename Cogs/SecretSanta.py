...
class SecretSanta(commands.Cog):
    ...
    async def getssrole(self, ctx):
        ...
        if arole.name[:1].lower() in vowels:
            msg = 'You need to be an **{}** to participate in Secret Santa.'.format(
                arole.name)
        else:
            msg = 'You need to be a **{}** to participate in Secret Santa.'.format(
                arole.name)
        return await ctx.send(Utils.suppressed(ctx, msg))
    ...
    async def sscreatechannels(self, ctx, *, category=None):
        ...
        category = DisplayName.channelForName(
            category_name, ctx.guild, "category")
        if not category:
            # Create it
    ...
    async def ssgenreport(self, ctx, *, category=None):
        ...
        json.dump(results, open(
            "Secret_Santa_{}.json".format(ctx.guild.id), "w"), indent=2)
        # Upload it
    ...
    async def ssapplyreport(self, ctx, *, url=None):
        ...
        for x in swap_list:
            ...
            try:
                ...
                channel = next(
                    (x for x in category.text_channels if x.name == str(to_user.id)), None)
            except:
            ...
            resolved.append(
                {"to": to_user, "from": from_user, "chan": channel})
