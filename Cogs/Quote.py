...
class Quote(commands.Cog):
    ...
    async def on_raw_reaction_add(self, payload):
        ...
        reaction = next(
            (x for x in message.reactions if str(x) == str(r)), None)
        if not reaction:
            return  # Broken for no reason?
        ...
        if len(reaction.message.embeds) and image == None:
            ...
            for e in reaction.message.embeds:
                d = e.to_dict()
                i = d.get("thumbnail", d.get(
                    "video", d.get("image", {}))).get("url", None)
                if not i:
                    continue
                image = i
                break
        ...
        e = {
            ...
            "description": msg + "\n\nSent by {} in {} | {} | {} UTC".format(
                ...
                "[Link](https://discord.com/channels/{}/{}/{})".format(
                    reaction.message.guild.id, reaction.message.channel.id, reaction.message.id),
    ...
    async def setquotereaction(self, ctx):
        ...
        backup_reaction = self.settings.getServerStat(
            ctx.guild, "QuoteReaction")
        self.settings.setServerStat(ctx.guild, "QuoteReaction", None)
        # Now we would wait...
