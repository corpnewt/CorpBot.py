...
class Giphy(commands.Cog):
    ...
    async def removegif(self, ctx, *, role: str = None):
        ...
        if not found_role:
            found_role = next(
                (x for x in promoArray if x["Name"].lower() == role.name.lower()), False)
        if found_role:
    ...
    async def listgif(self, ctx):
        ...
        if not len(promoSorted):
            roleText = "There are no gif roles set yet.  Use `{}addgif [role]` to add some.".format(
                ctx.prefix)
        ...
        for arole in promoSorted:
            ...
            roleText += "**{}** (removed from server)\n".format(
                arole["Name"]) if role is None else "**{}** (ID : `{}`)\n".format(role.name, arole["ID"])
    ...
    async def gif(self, ctx, *, gif=None):
        ...
        if gif == None:
            ...
            try:
                my_gif = random.choice(
                    list(self.giphy.search(phrase=gif, limit=20)))
            except Exception:
                my_gif = None
