...
class CogManager(commands.Cog):
    ...
    def __init__(self, bot, settings):
        self.preloads = ("Cogs.Utils", "Cogs.DisplayName",
                         "Cogs.Settings", "Cogs.Mute")
        self.bot = bot
        ...
        if extension == None:
            ...
            for x in self.preloads:
                if x in self.bot.extensions:
                    self.bot.dispatch("unloaded_extension",
                                      self.bot.extensions.get(x))
                    try:
                ...
                try:
                    ...
                    self.bot.dispatch("loaded_extension",
                                      self.bot.extensions.get(x))
                except:
                    print("{} failed to load!".format(x))
            # Assumes the prior 2 loaded correctly
                ...
                if ext.lower().endswith(".py") and not (ext.lower() in ["settings.py", "mute.py"]):
                    ...
                    try:
                        ...
                        if "Cogs."+ext[:-3] in self.bot.extensions:
                            self.bot.dispatch(
                                "unloaded_extension", self.bot.extensions.get("Cogs."+ext[:-3]))
                    ...
                    try:
                        ...
                        self.bot.dispatch(
                            "loaded_extension", self.bot.extensions.get("Cogs."+ext[:-3]))
                        cog_loaded += 1
                    except Exception as e:
                    ...
                    for e in to_reload:
                        ...
                        try:
                            ...
                            if "Cogs."+e[:-3] in self.bot.extensions:
                                self.bot.dispatch(
                                    "unloaded_extension", self.bot.extensions.get("Cogs."+e[:-3]))
                        ...
                        try:
                            ...
                            self.bot.dispatch(
                                "loaded_extension", self.bot.extensions.get("Cogs."+e[:-3]))
                            success += 1
                        except Exception as er:
    ...
    async def extension(self, ctx, *, extension=None):
        ...
        for e in self.bot.extensions:
            ...
            if len(cog_list):
                ...
                help_embed.add_field(name=", ".join(
                    cog_list), value=comm, inline=True)
            else:
                help_embed.add_field(
                    name="No Cogs", value="└─ 0 commands", inline=True)
    ...
    async def reload(self, ctx, *, extension=None):
        ...
        if extension == None:
            ...
            res_str = "*{}* of *{}* extensions reloaded successfully!".format(
                result[0], result[1])
    ...
    async def update(self, ctx):
        ...
        if os.name == 'nt':
            ...
            command = "which"
        try:
            p = subprocess.run(command + " git", shell=True, check=True,
                               stderr=subprocess.DEVNULL, stdout=subprocess.PIPE)
            git_location = p.stdout.decode(
                "utf-8").split("\n")[0].split("\r")[0]
        except:
            git_location = None
