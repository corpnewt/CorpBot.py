...
class DisableCommand(commands.Cog):
    ...
    async def message(self, message):
        ...
        if ctx.command.name in dis_com:
            ...
            admin_allow = self.settings.getServerStat(
                ctx.guild, "AdminDisabledAccess")
            badmin_allow = self.settings.getServerStat(
                ctx.guild, "BAdminDisabledAccess")
            ...
            if (is_admin and admin_allow) or (is_badmin and badmin_allow):
                return
            # React if needed
            to_react = self.settings.getServerStat(
                ctx.guild, "DisabledReactions")
            if to_react:
    ...
    def _get_our_comms(self):
        ...
        our_comm_list = [c.name for c in self.bot.get_cog(
            our_cog).get_commands()]
    ...
    async def disable(self, ctx, *, command_or_cog_name=None):
        ...
        msg = "All eligible passed commands are already disabled." if len(
            disabled) == 0 else ", ".join(sorted(disabled))
        title = "Disabled 1 Command" if len(
            disabled) == 1 else "Disabled {} Commands".format(len(disabled))
    ...
    async def enable(self, ctx, *, command_or_cog_name=None):
        ...
        if len(enabled):
            ...
            self.settings.setServerStat(
                ctx.guild, "DisabledCommands", dis_copy)
        # Now we give some output
        msg = "All eligible passed commands are already enabled." if len(
            enabled) == 0 else ", ".join(sorted(enabled))
        title = "Enabled 1 Command" if len(
            enabled) == 1 else "Enabled {} Commands".format(len(enabled))
        await Message.EmbedText(desc_head="```\n", desc_foot="```", color=ctx.author, description=msg, title=title).send(ctx)
    ...
    async def listdisabled(self, ctx):
        ...
        if not await Utils.is_admin_reply(ctx):
            return
        dis_com = self.settings.getServerStat(ctx.guild, "DisabledCommands")
        msg = "No commands have been disabled." if len(
            dis_com) == 0 else ", ".join(sorted(dis_com))
        title = "1 Disabled Command" if len(
            dis_com) == 1 else "{} Disabled Commands".format(len(dis_com))
        await Message.EmbedText(desc_head="```\n", desc_foot="```", color=ctx.author, description=msg, title=title).send(ctx)
    ...
    async def isdisabled(self, ctx, *, command_or_cog_name=None):
        ...
        for c in dis_com:
            if c in comm:
                disabled.append(c)
        if is_cog:
            title = "1 Command Disabled in {}".format(command_or_cog_name) if len(
                disabled) == 1 else "{} Commands Disabled in {}".format(len(disabled), command_or_cog_name)
            if len(disabled) == 0:
                msg = "None"
                footer = "0% disabled"
            else:
    ...
    async def disableall(self, ctx):
        ...
        # Give some output
        msg = "All eligible commands are already disabled." if len(
            disabled) == 0 else ", ".join(sorted(disabled))
        title = "Disabled 1 Command" if len(
            disabled) == 1 else "Disabled {} Commands".format(len(disabled))
        await Message.EmbedText(desc_head="```\n", desc_foot="```", color=ctx.author, description=msg, title=title).send(ctx)
    ...
    async def enableall(self, ctx):
        ...
        # Give some output
        msg = "All eligible commands are already enabled." if len(
            dis_com) == 0 else ", ".join(sorted(dis_com))
        title = "Enabled 1 Command" if len(
            dis_com) == 1 else "Enabled {} Commands".format(len(dis_com))
        await Message.EmbedText(desc_head="```\n", desc_foot="```", color=ctx.author, description=msg, title=title).send(ctx)
