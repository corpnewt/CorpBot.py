...
class UrbanDict(commands.Cog):
    ...
    async def _get_json_list(self, url):
        try:
            json_data = await DL.async_json(url, headers={'User-agent': self.ua})
        except:
            json_data = {}
    ...
    def _format_definition(self, entry):
        ...
        if entry.get("example"):
            lines = ["*{}*".format(x.strip()) if len(x.strip())
                     else "" for x in entry["example"].replace("*", "").split("\n")]
            value += "\n\n__Example(s):__\n\n{}".format("\n".join(lines))
        for match in set([match.group(0) for match in re.finditer(self.regex, value)]):
            value = value.replace(match, "__{}({})__".format(
                match, "https://www.urbandictionary.com/define.php?term={}".format(quote(match[1:-1]))))
        return value
    ...
    async def define(self, ctx, *, word: str = None):
        ...
        if not word:
            return await ctx.send('Usage: `{}define [word]`'.format(ctx.prefix))
