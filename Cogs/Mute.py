...
class Mute(commands.Cog):
            ...
            for entry in muteList:
                ...
                if member:
                    ...
                    self.loop_list.append(self.bot.loop.create_task(
                        self.checkMute(member, server, cooldown)))
        # Add a loop to remove expired mutes in the MuteList
        self.loop_list.append(
            self.bot.loop.create_task(self.mute_list_check()))
    ...
    async def checkMute(self, member, server, cooldown):
        ...
        if cd == None:
            ...
            if timeleft > 0:
                ...
                self.loop_list.append(self.bot.loop.create_task(
                    self.checkMute(member, server, cd)))
                self._remove_task(task)
                return
    ...
    async def _mute(self, member, server, cooldown=None, muted_by=None):
        ...
        entry = next((x for x in muteList if str(
            x["ID"]) == str(member.id)), False)
        if entry:
            entry["Cooldown"] = cooldown
        else:
            muteList.append(
                {'ID': member.id, 'Cooldown': cooldown, 'Added': int(time.time())})
        # Save the results
    ...
    async def _ask_perms(self, ctx, mute_role, desync=False, show_count=True):
        ...
        try:
            sync_response = await self.bot.wait_for('message', timeout=60, check=check_answer)
        except:
    ...
    async def _sync_perms(self, ctx, mute_role, desync=False):
        ...
        if guild is None:
            return  # Got sent some wonky values, I guess...
        for channel in guild.channels:
            ...
            if desync:
                perm_check = any(
                    x[0] in self.mute_perms and x[1] != None for x in overs)
            else:
            ...
            if perm_check:  # We qualify - set our perms as needed
                other_perms = any(
                    x[0] not in self.mute_perms and x[1] != None for x in overs)
                overs.send_messages = overs.add_reactions = overs.speak = None if desync else False
                try:
                    await channel.set_permissions(mute_role, overwrite=overs if other_perms or not desync else None)
                except:
                    pass
    ...
    async def setmuterole(self, ctx, *, role=None):
        ...
        if role:
            ...
            if not target_role:
                return await ctx.send("That role doesn't exist - you can create a new mute role with `{}createmuterole [role_name]` though.".format(ctx.prefix))
        try:
            mute_role = ctx.guild.get_role(
                int(self.settings.getServerStat(ctx.guild, "MuteRole")))
        except:
            mute_role = None
    ...
    async def muterole(self, ctx):
        ...
        try:
            mute_role = ctx.guild.get_role(int(role))
        except:
            mute_role = None
        if not mute_role:
            return await ctx.send("The prior mute role (ID: `{}`) no longer exists.  You can set one with `{}setmuterole [role]` - or have me create one with `{}createmuterole [role_name]`".format(role, ctx.prefix, ctx.prefix))
    ...
    async def createmuterole(self, ctx, *, role_name=None):
        ...
        if not await Utils.is_bot_admin_reply(ctx):
            return
        if not role_name:
            return await ctx.send("Usage: `{}createmuterole [role_name]`".format(ctx.prefix))
        ...
        try:
            mute_role = await ctx.guild.create_role(name=role_name, reason="Mute role created by {}#{}".format(ctx.author.name, ctx.author.discriminator))
        except Exception as e:
            return await message.edit(content="Role create failed with error:\n```\n{}\n```".format(e))
        ...
        for channel in ctx.guild.channels:
            ...
            if hasattr(channel, "permissions_synced"):  # Implemented in 1.3.0 of discord.py
                if channel.permissions_synced:
                    channel = channel.category  # Get the category if we're synced
            overs = channel.overwrites_for(mute_role)
            if not all([x == False for x in (overs.send_messages, overs.add_reactions, overs.speak)]):
                ...
                try:
                    await channel.set_permissions(mute_role, overwrite=overs)
                except:
                    pass
    ...
    async def mute(self, ctx, *, member=None, cooldown=None):
        ...
        if await Utils.is_bot_admin_reply(ctx, member=member, message="You can't mute other admins or bot-admins.", message_when=True):
            return
        ...
        mess = await ctx.send("Muting...{}".format(
            "" if role else " You can set up a mute role with `{}setmuterole [role]` or `{}createmuterole [role_name]` for a **much faster** muting experience.".format(
    ...
    async def ismuted(self, ctx, *, member=None):
        ...
        try:
            ...
            mute_role = None
        item = next((x for x in self.settings.getServerStat(
            ctx.guild, "MuteList", []) if str(x["ID"]) == str(member.id)), None)
        ...
        for channel in ctx.guild.channels:
            ...
            if hasattr(channel, "permissions_synced"):  # Implemented in 1.3.0 of discord.py
                if channel.permissions_synced:
                    channel = channel.category  # Get the category if we're synced
            # Get any overrides for the user
