...
class Hw(commands.Cog):
    ...
    async def pcpp(self, ctx, url=None, style=None, escape=None):
        ...
        usage = "Usage: `{}pcpp [url] [style=normal, md, mdblock, bold, bolditalic] [escape=yes/no (optional)]`".format(
            ctx.prefix)
        if not style:
            style = 'normal'
        if not url:
    ...
    async def delhw(self, ctx, *, build=None):
        ...
        for b in buildList:
            if b['Name'].lower() == build.lower():
                ...
                self.settings.setGlobalUserStat(
                    ctx.author, "Hardware", buildList)
    ...
    async def edithw(self, ctx, *, build=None):
        ...
        if ctx.guild:
            # Not a pm
            hwChannel = self.settings.getServerStat(
                ctx.guild, "HardwareChannel")
                ...
                for build in buildList:
                    ...
                    for line in bParts.splitlines():
                        if buildStr.lower() in line.lower():
                            entries.append({"name": "{}. {}".format(
                                len(entries)+1, build["Name"]), "value": line})
                if len(entries):
                    # We're in business
    ...
    async def hw(self, ctx, *, user: str = None, build=None):
            ...
            for j in range(len(parts)):
                ...
                if memFromName:
                    ...
                    if buildList == None:
                        buildList = []
                    buildList = sorted(
                        buildList, key=lambda x: x['Name'].lower())
                    try:
        ...
        msg_head = "__**{}'s {}:**__\n\n".format(
            DisplayName.name(memFromName), buildParts['Name'])
        msg = msg_head + buildParts['Hardware']
        if len(msg) > 2000:  # is there somwhere the discord char count is defined, to avoid hardcoding?
            # if the header pushes us over the limit, omit it and send just the string
            msg = buildParts['Hardware']
        await ctx.send(Utils.suppressed(ctx, msg))
    ...
    async def listhw(self, ctx, *, user=None):
        ...
        if not len(buildList):
            ...
            return await ctx.send(msg)
        items = [{"name": "{}. {}".format(i, x["Name"]+(" (Main Build)" if x["Main"] else "")),
                  "value":Utils.truncate_string(x["Hardware"])} for i, x in enumerate(buildList, start=1)]
    ...
    async def lhw(self, ctx, *, user=None):
        ...
        if not len(buildList):
            msg = '*{}* has no builds on file!  They can add some with the `{}newhw` command.'.format(
                DisplayName.name(member), ctx.prefix)
            return await ctx.send(msg)
        desc = "\n".join([Utils.truncate_string("{}. {}".format(
            i, x["Name"]+(" (Main Build)" if x["Main"] else ""))) for i, x in enumerate(buildList, start=1)])
        return await PickList.PagePicker(
            title="{}'s Builds ({:,} total)".format(
                DisplayName.name(member), len(buildList)),
            description=desc,
            ctx=ctx
        ).pick()
