...
class Fliptime(commands.Cog):
    ...
    async def message(self, message):
        ...
        back_list = [')', '）']
        # (╯⁰ㅁ⁰）╯︵ ㅗㅡㅗ
        face = any(ext in conts for ext in front_list) and any(
        ...
        if face and table:
            ...
            if int(previousCooldown) > currentTime:
                ...
                res = '┬─┬ ノ( ゜-゜ノ)  *{}*, I understand that you\'re frustrated, but we still don\'t flip tables here.  Why don\'t you cool down for *{}* instead.'.format(
                    DisplayName.name(message.author), coolText)
            else:
                # Not cooling down - start it
                coolText = ReadableTime.getReadableTimeBetween(
                    currentTime, cooldownFinal)
                res = '┬─┬ ノ( ゜-゜ノ)  *{}*, we don\'t flip tables here.  You should cool down for *{}*'.format(
                    DisplayName.name(message.author), coolText)
