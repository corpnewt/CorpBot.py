...
def getUserTime(member, settings, time=None, strft="%Y-%m-%d %I:%M %p", clock=True, force=None):
    ...
    offset = force if force else settings.getGlobalUserStat(
        member, "TimeZone", settings.getGlobalUserStat(member, "UTCOffset", None))
    if offset == None:
        # No offset or tz - return UTC
        t = getClockForTime(time.strftime(
            strft)) if clock else time.strftime(strft)
...
def getTimeFromTZ(tz, t=None, strft="%Y-%m-%d %I:%M %p", clock=True):
    ...
    zone = next((pytz.timezone(x)
                for x in pytz.all_timezones if x.lower() == tz.lower()), None)
    if zone == None:
        return None
    zone_now = datetime.datetime.now(zone) if t == None else pytz.utc.localize(
        t, is_dst=None).astimezone(zone)
    ti = getClockForTime(zone_now.strftime(
        strft)) if clock else zone_now.strftime(strft)
