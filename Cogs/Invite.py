...
class Invite(commands.Cog):
    ...
    async def onserverjoin(self, server):
        ...
        if not self.settings.getGlobalStat("AllowServerJoin", True):
            ...
            temp = next(
                (x for x in self.temp_allowed if x[0] == server.id), None)
            if temp:
    ...
    def _check_whitelist(self):
        ...
        self.settings.setGlobalStat("ServerWhitelist", None if self.settings.getGlobalStat(
            "AllowServerJoin", True) else [x.id for x in self.bot.guilds])
    ...
    def _whitelist_server(self, guild_id=None):
        ...
        current_whitelist = [] if not isinstance(
            current_whitelist, (list, tuple)) else current_whitelist
    ...
    def _unwhitelist_server(self, guild_id=None):
        ...
        if not guild_id:
            return
        current_whitelist = self.settings.getGlobalStat("ServerWhitelist", [])
        current_whitelist = [] if not isinstance(current_whitelist, (list, tuple)) else [
            x for x in current_whitelist if not x == guild_id]
        self.settings.setGlobalStat(
            "ServerWhitelist", current_whitelist if len(current_whitelist) else None)
    ...
    async def on_loaded_extension(self, ext):
        ...
        if self.settings.getGlobalStat("AllowServerJoin", True):
            return  # No need to check - not restricting
        print("Verifying server whitelist...")
        current_whitelist = self.settings.getGlobalStat(
            "ServerWhitelist", None)
        ...
        for x in bail_list:
            ...
            try:
                ...
                print(" --> I couldn't leave {} :(".format(x))
    ...
    async def invite(self, ctx, invite_url=None):
        ...
        if server and any(x for x in self.temp_allowed if x[0] == server.id):
            ...
            return await ctx.send('Invite me to {} with this link: \n<{}>'.format(
                ...
                discord.utils.oauth_url(
                    self.bot.user.id, permissions=discord.Permissions(permissions=8), guild=server)
            ))
    ...
    async def requestjoin(self, ctx, invite_url=None):
        ...
        temp = next(
            (x for x in self.current_requests if x[1].id == invite.guild.id), None)
        if temp:
        ...
        temp = next(
            (x for x in self.temp_allowed if x[0] == invite.guild.id), None)
        if temp:
        ...
        request = (ctx.author, invite.guild,
                   time.time()+self.request_time, ctx)
    ...
    async def approvejoin(self, ctx, server_id=None):
        ...
        if request:
            await request[3].send("{}, your request for me to join {} has been approved for the next {}.  You can invite me with this link:\n<{}>".format(
                ...
                discord.utils.oauth_url(self.bot.user.id, permissions=discord.Permissions(
                    permissions=8), guild=request[1])
    ...
    async def canjoin(self, ctx, *, yes_no=None):
        ...
        if yes_no == None:
            msg = "{} currently *{}.*".format(setting_name,
                                              "enabled" if current else "disabled")
        elif yes_no.lower() in ["yes", "on", "true", "enabled", "enable"]:
            yes_no = True
            msg = "{} {} *enabled*.".format(setting_name,
                                            "remains" if current else "is now")
        elif yes_no.lower() in ["no", "off", "false", "disabled", "disable"]:
            yes_no = False
            msg = "{} {} *disabled*.".format(setting_name,
                                             "remains" if not current else "is now")
        else:
    ...
    async def block(self, ctx, *, server: str = None):
        ...
        for serv in serverList:
            if str(serv).lower() == server.lower():
                ...
                msg = "*{}* is already blocked!".format(
                    Nullify.escape_all(serv))
    ...
    async def unblock(self, ctx, *, server: str = None):
        ...
        serverList = self.settings.getGlobalStat('BlockedServers', [])
        serverTest = [x for x in serverList if not str(
            x).lower() == server.lower()]
        if len(serverList) != len(serverTest):
            ...
            return await ctx.send(msg)
        # Not found
        msg = "I couldn't find *{}* in my blocked list.".format(
            Nullify.escape_all(server))
        await ctx.send(msg)
    ...
    async def blocked(self, ctx):
        ...
        if not len(serverList):
            ...
            msg = "__Currently Blocked:__\n\n{}".format(
                ', '.join([Nullify.escape_all(x) for x in serverList]))
        await ctx.send(msg)
