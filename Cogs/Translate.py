...
class Translate(commands.Cog):
    ...
    async def langlist(self, ctx):
        ...
        for lang in googletrans.LANGCODES:
            description += "**{}** - {}\n".format(
                string.capwords(lang), googletrans.LANGCODES[lang])
    ...
    async def detectlang(self, ctx, *, text):
        ...
        if text == None:
            return await ctx.send("Usage: `{}detectlang [text to identify]`".format(ctx.prefix))
        ...
        await Message.EmbedText(
            ...
            description="Detected **{}** ({}) with {:.0%} confidence.".format(
                string.capwords(googletrans.LANGUAGES.get(
                    lang_detect.lang.lower(), "Martian?")),
    ...
    async def tr(self, ctx, *, translate=None):
        ...
        await Message.EmbedText(
            ...
            footer="{} --> {} - Powered by Google Translate".format(
                string.capwords(from_lang_name), string.capwords(to_lang_name))
        ).send(ctx)
