...
class PagePicker(Picker):
    def __init__(self, **kwargs):
        ...
        # Used defaults of 10 and 20 respectively
        self.max = kwargs.get("max", 10 if self.list else 20)
        # Must be between 1 and 25 for fields, 1 and 2048 for desc rows
        max_val = 25 if self.list else 2048
        ...
        if self.newline_split:
            ...
            for line in self.description.split("\n"):
                ...
                if len(line) > adj_max:  # The line itself is too long
                    if len(curr):
                        chunks.append(self.d_header+curr+self.d_footer)
                    ...
                    chunks.append(self.d_header+(test if len(test)
                                  == adj_max else curr)+self.d_footer)
    ...
    async def pick(self):
        ...
        embed = {
            ...
            # We can leave it a huge number for desc without issue - never pm automagically
            "pm_after": 25,
        ...
        while True:
            ...
            page = 0 if ind == 0 else page-1 if ind == 1 else page + \
                1 if ind == 2 else pages-1 if ind == 3 else page
            if ind == 4:
                # User selects a page
            ...
            if not 0 <= page < pages:  # Don't update if we hit the end
                ...
                continue
            embed["fields" if self.list else "description"] = self._get_page_contents(
                page)
