...
class Music(commands.Cog):
    ...
    def _get_mention(self, info):
        try:
            return info["added_by"].mention
        except:
            return info.get("added_by", "Unknown")
    ...
    async def on_next_song(self, ctx, error=None):
        ...
        await Message.Embed(
            ...
            color=ctx.author if not data.info.get(
                "ctx") else data.info["ctx"].author,
            url=data.uri,
            thumbnail=data.thumb,
            delete_after=delay
        ).send(ctx)
    ...
    async def playing(self, ctx, *, moons=None):
        ...
        await Message.Embed(
            ...
            description="Requested by {} -- Volume at {}%".format(
                self._get_mention(data.info), cv),
            color=ctx.author,
            fields=[
    ...
    async def unskip(self, ctx):
        ...
        if not ctx.author.id in skips:
            return await Message.EmbedText(title="♫ You haven't voted to skip this song!", color=ctx.author, delete_after=delay).send(ctx)
